...
;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		init: function () {
			...
			if ( self.data('ui-theme') && self.data('ui-theme-id') ) {
				...
				if ( closestParentUI.length ) {
					this.ui_theme_id = 'ipt-uif-custom-' + closestParentUI.data( 'ui-theme-id' );
				}
			}
		...
		conditionalHideElement: function() {
			...
			if ( ! _self.hasClass('iptUIFCHidden') ) {
				// Reset all data
				if ( _self.find( '.ipt_fsqm_payment_method_radio' ).length ) {
					if ( _self.find( '.ipt_fsqm_payment_method_radio .ipt_uif_radio' ).filter(':checked').length ) {
						_self.find( '.ipt_fsqm_payment_method_radio' ).data( 'iptfsqmpp', _self.find( '.ipt_fsqm_payment_method_radio .ipt_uif_radio' ).filter(':checked').val() );
					}
				}
				...
				if ( _self.find('input[type="text"], textarea, input[type="password"], input[type="number"], input[type="email"], input[type="tel"]').length ) {
					_self.find('input[type="text"], textarea, input[type="password"], input[type="number"], input[type="email"], input[type="tel"]').val('').trigger('change');
				}
				...
				if ( _self.find('.ipt_uif_jsignature_reset').length ) {
					_self.find('.ipt_uif_jsignature_reset').trigger('click');
				}

				if ( _self.find('.ipt-eform-hidden-field').length ) {
					_self.find('.ipt-eform-hidden-field').each( function() {
						$(this).data( 'eformDefaultValue', $(this).val() );
						$(this).val( '' );
						$(this).trigger( 'change' );
					} );
				}

				if ( _self.find( '.ipt-eform-trumbowyg' ).length ) {
					_self.find( '.ipt-eform-trumbowyg' ).trumbowyg('empty');
		...
		validateLogic: function( base, logics ) {
			...
			for ( logic_id in logics ) {
				...
				switch( check_type ) {
					...
					case 'range' :
						compare_source = [that.intelParseFloat( conditional_div.find('input.ipt_uif_slider.slider_range').val() ), that.intelParseFloat( conditional_div.find('input.ipt_uif_slider.slider_range').next('input').val() )];
						logic.value = that.intelParseFloat( logic.value );
						break;
					...
					case 'grading' :
						...
						conditional_div.find('input.ipt_uif_slider.slider_range').each(function() {
							...
							if ( $(this).next('input').val() ) {
								compare_source[compare_source.length] = that.intelParseFloat( $(this).next('input').val() );
							}
						});
						logic.value = that.intelParseFloat( logic.value );
						break;
		...
		edApplyjSignature: function() {
			...
			this.jElement.on('change', '.ipt_uif_jsignature_pad', function( e ) {
				...
				signatureText = elm.siblings('.ipt_uif_jsignature_input').val(signatureData).trigger('change').trigger('blur');
			...
			this.jElement.on('iptUIFCShow tabsactivate fsqm.jSignatureRedraw', function(e) {
				...
				setTimeout( $.proxy( function() {
					$(this).find('.ipt_uif_jsignature_pad').each(function() {
						...
						if ( $(this).is(':visible') && e.type === 'iptUIFCShow' && ! $( e.target ).find( '.ipt_uif_jsignature_pad' ).is( $( this ) ) ) {
							return true;
						}
						// Not inside a hidden tab
						// Not inside a hidden conditional element
						if ( $(this).closest('.ui-tabs-panel[aria-hidden="true"]').length === 0 && $(this).closest('.iptUIFCHidden').length === 0 ) {
							// Get the data
							var signData = $(this).prev('.ipt_uif_jsignature_input').val();
							...
							setTimeout(function() {
								// Destryo the canvas
								var sig = _self.find('canvas').data('jSignature.this');
								if(sig) {
								...
								if ( signData !== '' && signData !== null && signData !== undefined && signData != 'image/jsignature;base30,' ) {
									_self.jSignature('setData', signData, 'base30');
								}
							}, 100);
						}
					});
		...
		_updateProvince: function( countryAutoComplete, provincePreset ) {
			...
			provinceAutoComplete.addClass( 'working' );
			...
			$.getJSON( iptPluginUIFFront.ajaxurl, {
				...
				provinceAutoComplete.removeClass( 'working' );
			} )
		},
