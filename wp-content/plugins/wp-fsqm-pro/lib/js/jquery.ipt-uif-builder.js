...
(function($) {
	...
	var methods = {
		...
		applyBuilder : function() {
			...
			// Sticky kit not needed because it is glitchy
			// $( this ).on( 'SDAAdd.eform SDADelete.eform tabsactivate', function() {
			// 	setTimeout( function() {
		...
		builderSettingsOpen : function(settings_box, container, origin) {
			...
			$(settings_box).parent().stop(true, true).css({height : 'auto'}).hide().fadeIn('fast', function() {
				...
				// No need for sticky kit because it is glitchy
				// setTimeout( function() {
				// 	var settingsContainer = $( '#ipt-eform-builder-settings-wrap' ),
				// 	builderSticky = settingsContainer.data( 'builderSticky' );
				// 	if ( builderSticky ) {
				// 		$( document.body ).trigger( 'sticky_kit:recalc' );
				// 	} else {
				// 		// Add Sticky
				// 		settingsContainer.stick_in_parent( {
				// 			offset_top: 48
				// 		} );
				// 		settingsContainer.data( 'builderSticky', true );
				// 	}
				// }, 100 );
			});
