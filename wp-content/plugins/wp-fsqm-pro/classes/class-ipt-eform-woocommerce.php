...
class IPT_eForm_WooCommerce {
	...
	public function add_cart_item( $cart_item ) {
		...
		if ( isset( $cart_item['eform_cart'] ) && true == $cart_item['eform_cart'] ) {
			// Adjust pricing
			if ( isset( $cart_item['eform_price'] ) ) {
	...
	public function get_cart_item_from_session( $cart_item, $values ) {
		...
		if ( isset( $values['eform_cart'] ) && true == $values['eform_cart'] ) {
			...
			$cart_item['eform_attr'] = $values['eform_attr'];
	 ...
	 */
	public function add_to_cart( $data_id, $prod_id, $data_arr ) {
		...
		$woocommerce->cart->add_to_cart( $prod_id, 1, 0, array(), $data_arr );
	}
	...
	public function after_order_changed( $order_id, $old_status, $new_status ) {
		...
		foreach ( $items as $item ) {
			...
			$data = new IPT_FSQM_Form_Elements_Data( $item['item_meta']['_eform_data_id'][0] );
