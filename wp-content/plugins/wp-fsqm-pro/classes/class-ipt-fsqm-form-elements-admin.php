...
?>
	...
	<div id="ipt-eform-builder-droppables-container" data-margin-top="32">
...
<table class="form-table">
	<tbody>
		...
		<tr>
			...
			<td>
				<?php $this->ui->help_head(); ?>
					...
					<p><?php printf( __( 'An updated list can always be found <a href="%1$s" target="_blank">here</a>.', 'ipt_fsqm' ), 'https://ipanelthemes.com/kb/fsqm/form-submission-related/available-format-strings-custom-notifications/' ); ?></p>
				<?php $this->ui->help_tail(); ?>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th><?php $this->ui->generate_label( 'settings[user][math_format]', __( 'Add format strings for mathematical elements', 'ipt_fsqm' ) ); ?></th>
			<td>
				<?php $this->ui->toggle( 'settings[user][math_format]', __( 'Yes', 'ipt_fsqm' ), __( 'No', 'ipt_fsqm' ), $op['math_format'] ); ?>
			</td>
			<td>
				<?php $this->ui->help( __( 'If enabled, then you will have format strings like <code>%MATH{id}%</code> where <code>{id}</code> is the key of the form element (visible in form builder). These format strings would be replaced by values of the mathematical elements. For example, <code>%MATH3%, %MATH14%</code> etc. Note: This will also work for admin notification.', 'ipt_fsqm' ) ); ?>
			</td>
		</tr>
		<tr>
...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<?php $this->ui->toggle( 'settings[payment][woocommerce][enabled]', __( 'Yes', 'ipt_fsqm' ), __( 'No', 'ipt_fsqm' ), $op['enabled'], '1', false, true, array(
					'condid' => 'ipt_fsqm_intg_wc_msg,ipt_fsqm_intg_wc_pid_wrap,ipt_fsqm_intg_wc_math_wrap,ipt_fsqm_intg_wc_rd_wrap,ipt_fsqm_intg_wc_pfs_wrap,ipt_fsqm_intg_wc_cpid_wrap,ipt_fsqm_intg_wc_cpidl_wrap',
				) ); ?>
			</td>
			<td>
		...
		<tr id="ipt_fsqm_intg_wc_msg">
			<td colspan="3">
				<?php $this->ui->msg_update( __( 'WooCommerce Payment works in a different way. eForm would just add the product you have selected with modified pricing, as calculated from the form. The variables in the mathematical formula will be used to create attributes on the go. eForm will not store any payment information directly as with PayPal or Stripe payment. The checkout functionality would be managed by WooCommerce and eForm would behave just like a form, not a checkout page. You do not need to enable the Payment settings for WooCommerce integration to work. Also you do not need to place a payment element inside the form. Just place a mathematical element and this will work straight out of the box. If you have conditional logic enabled on the mathematical element, and if the element stays conditionally hidden, then WooCommerce integration would not execute.', 'ipt_fsqm' ) ); ?>
			</td>
		</tr>
		<tr id="ipt_fsqm_intg_wc_pid_wrap">
			...
			<td>
				<?php $this->ui->text( 'settings[payment][woocommerce][product_id]', $op['product_id'], __( 'Product ID', 'ipt_fsqm' ) ); ?>
			</td>
		...
		<tr id="ipt_fsqm_intg_wc_math_wrap">
			...
			<td>
				<?php $this->ui->text( 'settings[payment][woocommerce][mathematical]', $op['mathematical'], __( 'Math ID', 'ipt_fsqm' ) ); ?>
			</td>
...
<table class="form-table">
	<tbody>
		...
		<tr id="ipt_fsqm_formhandler_metatitle_wrap">
			...
			<td>
				<p class="description"><?php printf( __( '%1$s will always be sent with the corresponding values.', 'ipt_fsqm' ), '<code>data_id, submission_date, user_id, f_name, l_name, email, score, max_score, ip, remarks, referrer, url_track, time, link</code>' ); ?></p>
			...
			<td><?php $this->ui->help_head(); ?>
				<p><?php _e( 'Add the query parameters you want to pass to the URL. You must specify a valid URL query key, otherwise the function might fail. <code>data_id, submission_date, user_id, f_name, l_name, email, score, max_score, ip, remarks, referrer, url_track, time, link</code> are always passed.', 'ipt_fsqm' ); ?></p>
		...
		<table class="form-table">
			<tbody>
				<tr>
					...
					<td>
						<?php $this->ui->help( __( 'If you want the submitter to get subscribed to one or more MailPoet list, please enable it here. After this, make sure you add the Email(DB) field to the form.', 'ipt_fsqm' ) ); ?>
					</td>
				</tr>
...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<?php $this->ui->help( __( 'If you want the submitter to get subscribed to one or more MyMail list, please enable it here. After this, make sure you add the Email(DB) field to the form.', 'ipt_fsqm' ) ); ?>
			</td>
		</tr>
...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<?php $this->ui->help( __( 'If you want the submitter to get subscribed to a <a href="https://sendy.co">Sendy.co</a> Newsletter list, please enable it here. After this, make sure you add the Email(DB) field to the form.', 'ipt_fsqm' ) ); ?>
			</td>
		</tr>
...
<table class="form-table">
	<tbody>
		<tr>
			...
			<td>
				<?php $this->ui->help( __( 'If you want the submitter to get subscribed to a <a href="http://www.activecampaign.com/">Active Campaign</a> Newsletter list, please enable it here. After this, make sure you add the Email(DB) field to the form.', 'ipt_fsqm' ) ); ?>
			</td>
		</tr>
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td>
							<?php $this->ui->textarea( $name_prefix . '[settings][default]', $data['settings']['default'], __( 'None', 'ipt_fsqm' ) ); ?>
						</td>
			...
			<table class="form-table">
				<tbody>
					<tr>
						...
						<td><?php $this->ui->help( __( 'Set the type of value the hidden field will get. It can be based on URL parameter or user meta key. You can even make it constant by setting it to predefined and defining a default value. In case of <strong>If logged in</strong> the default value would be set if the user is logged in. Otherwise no value (effectively empty value) would be set. You can use this behavior to apply conditional logic on other elements too.', 'ipt_fsqm' ) ); ?></td>
					</tr>
					<tr>
						<th><?php $this->ui->generate_label( $name_prefix . '[settings][default]', __( 'Default Value', 'ipt_fsqm' ) ); ?></th>
						<td>
