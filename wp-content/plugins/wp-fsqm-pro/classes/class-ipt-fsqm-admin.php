...
class IPT_FSQM_About extends IPT_FSQM_Admin_Base {
	...
	/*==========================================================================
	 * Interacting with iPanelThemes JSON API
	 *========================================================================*/

	public function get_ipt_fsqm_json() {
		if ( false === ( $ipt_fsqm_json = get_transient( 'ipt_fsqm_json' ) ) ) {
			// It wasn't there, so regenerate the data and save the transient
			// Remote call to http://ipanelthemes.com/wp-json/acf/v2/page/304/
			// And reform version and addons
			// Then return
			$do_not_set = false;
			$ipt_fsqm_api = wp_remote_get( 'https://ipanelthemes.com/wp-json/ipt-api/v1/fsqm/' );
			...
			if ( ! $do_not_set ) {
				...
				// Why make the site slow because of iPanelThemes.com errors?
...
<div class="wrap about-wrap">
	<h1><?php printf( __( 'Welcome to eForm - %s', 'ipt_fsqm' ), IPT_FSQM_Loader::$version ); ?></h1>
	...
	<div class="wp-badge" style="background-image: url(<?php echo plugins_url( '/static/common/images/fsqm-logo-white.png', IPT_FSQM_Loader::$abs_file ) ?>); background-size: 128px 128px; background-position: center;"></div>
	...
	<div id="fsqm-whats-new">
		<div class="ipt_uif" style="margin:10px 0">
			<div class="golden-video-wrap">
				<iframe src="https://www.youtube.com/embed/3FLpMtH9RG8" frameborder="0" allowfullscreen></iframe>
			</div>
		</div>

		<h2><?php printf( __( 'eForm<sup style="font-size: 0.5em;">v%1$s</sup> - WordPress Form Builder', 'ipt_fsqm' ), IPT_FSQM_Loader::$version ); ?></h2>
		<p>
		...
		<p>
			<?php _e( 'Thank you for believing - <a href="https://ipanelthemes.com">Team @ iPanelThemes</a>.', 'ipt_fsqm' ); ?>
		</p>

		<h3><?php _e( 'Milestone Release', 'ipt_fsqm' ); ?></h3>
		...
		<p><?php _e( '<strong>Version 3.3.1</strong> improved admin backend color scheme.' ) ?></p>
		<p><?php _e( '<strong>Version 3.3.0</strong> improved Reports & Analytics, added Trends Widget, Form Reset for continous submissions, Summary Table customization.' ) ?></p>
		<p><?php _e( '<strong>Version 3.2.0</strong> updated shortcode generator buttons, added google analytics events tracking, added container auto progress, added mailpoet integration, added conditional logic for all integrations.', 'ipt_fsqm' ) ?></p>
		<p><?php _e( '<strong>Version 3.1.0</strong> added Active Campaign integration, improved performance with progress autosave, fixed payment form cc issue, added referer to admin email and some significant work on translations.', 'ipt_fsqm' ) ?></p>
		<p><?php _e( '<strong>Version 3.0.0</strong> introduces payment system, new integrations and signature element.', 'ipt_fsqm' ); ?> </p>
		<hr />
		...
		<div class="feature-section two-col">
			<div class="col">
				...
				<p><a href="https://ipanelthemes.com/kb/fsqm/payment-system/woocommerce-integration-eform/" target="_blank"><?php _e( 'Read More', 'ipt_fsqm' ); ?></a></p>
			</div>
			<div class="col">
		...
		<div class="feature-section two-col">
			<div class="col">
				...
				<p><a href="https://ipanelthemes.com/kb/fsqm/wp-core-integrations/" target="_blank"><?php _e( 'Read More', 'ipt_fsqm' ); ?></a></p>
			</div>
			<div class="col">
			...
			<h2><?php _e( 'Under the Hood', 'ipt_fsqm'); ?></h2>
				<div class="under-the-hood three-col">
					<div class="col">
						...
						<p><?php _e( 'eForm now comes with a comprehensive <a href="http://ipanelthemes.com/kb/fsqm/fsqm-api/" target="_blank">Developer\'s API</a>. If you wish to extend eForm with newer elements or want to integrate with your mailing system, it is now possible without touching the core.', 'ipt_fsqm' ); ?></p>
					</div>
					<div class="col">
...
<ul class="ipt_uif_ul_menu">
	<li><a href="http://ipanelthemes.com/kb/fsqm/fsqm-video-tutorials/"><i class="ipt-icomoon-play"></i> <?php _e( 'Getting Started', 'ipt_fsqm' ) ?></a></li>
	<li><a href="http://ipanelthemes.com/kb/fsqm/"><i class="ipt-icomoon-file3"></i> <?php _e( 'Documentation', 'ipt_fsqm' ); ?></a></li>
	<li><a href="http://ipanelthemes.com/kb/support/forum/wordpress-plugins/wp-feedback-survey-quiz-manager-pro/"><i class="ipt-icomoon-support"></i> <?php _e( 'Get Support', 'ipt_fsqm' ); ?></a></li>
</ul>
			...
			<table class="form-table">
				<tbody>
					...
					<tr>
						...
						<td><?php $this->ui->help( __( 'What to show when the form is submitted. ', 'ipt_fsqm' ) . sprintf( __( 'An updated list can always be found <a href="%1$s" target="_blank">here</a>.', 'ipt_fsqm' ), 'https://ipanelthemes.com/kb/fsqm/form-submission-related/available-format-strings-custom-notifications/' ) ); ?></td>
					</tr>
				</tbody>
			</table>
			...
			<tr>
				...
				<td>
					<?php $this->ui->help_head(); ?>
						...
						<p><?php _e( 'More instructions can be found <a href="https://ipanelthemes.com/kb/?p=9859">here</a>.', 'ipt_fsqm' ); ?></p>
					<?php $this->ui->help_tail(); ?>
				</td>
			</tr>
			<tr>
...
class IPT_FSQM_Category_Table extends WP_List_Table {
	...
	public function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			case 'forms' :
				return sprintf( '<a class="view" href="admin.php?page=ipt_fsqm_all_forms&cat_id=%1$d">%1$s</a>', $item['id'] );
				break;
	...
	public function prepare_items() {
		...
		// Prepare our query
		$query = "SELECT c.id id, c.name name, c.description description, COUNT( f.id ) forms FROM {$ipt_fsqm_info['category_table']} c LEFT JOIN {$ipt_fsqm_info['form_table']} f ON f.category = c.id GROUP BY f.category";
