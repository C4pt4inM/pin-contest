...
;(function ( $, window, document, undefined ) {
	...
	Plugin.prototype = {
		...
		initVariables: function() {
			...
			// State flags
			this.on_last_page = false; // This would also be true if form type is simple container
		...
		_initTimer: function() {
			var that = this, i, pageTime, totalTime;
			this._reInitTimer();
			// If overall timer
			...
			// If page specific timer
				...
				// To abide by the timer
					...
					that.main_tab.on( 'tabsactivate', this.__activeTabTimerRef );
				}
			// Incorrect settings
			} else {
				this._destroyTimer();
		...
		_processSubmission: function() {
			...
			$.post( iptFSQM.ajaxurl, data, function( response ) {
				...
				if ( response.success === true ) {
					...
					// Set Cookie
					try {
						var totalSubmission = parseInt( $.cookie( 'eform-submission-' + that.form_id ), 10 );
